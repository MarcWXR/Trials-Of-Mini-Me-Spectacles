...
The project is a puzzle platformer inspired by games I loved playing as a kid. The current set up is a basic time challenge to make it to the top in the fastest possible time. It starts when the player sets foot on the first platform and ends with a particle burst when the player reaches the top.

# Features
The hand interaction mechanic is very simple and I set up with wired parameters. For instance, moving a platform up and down on the Y axis controls the rotation of the balance beams. You can easily duplicate these and tweak the settings. 
...
All of the scripts are commented and easy to configure because that's the way I like it! :-)
...
The Spectacles Bitmoji Character Controller set up did not have a real jump configured, it only played the jump animation so I copied the relevant code across from the Snap Lens example project and added it to this one. Hopefully this makes it a lot easier for others to create these types of games in anticipation of the bluetooth game controller API moving from experimental status.
